...
fn test_regression_against_json() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for expected in expected_results {
        ...
        for (i, (actual, exp)) in actual_faces.iter().zip(expected_faces.iter()).enumerate() {
            ...
            // Compare Score
            assert_approx_eq(actual.score, exp.score, &format!("{} Score", ctx));

            // Compare BBox
            assert_approx_eq(actual.bbox.x1, exp.bbox.x1, &format!("{} BBox x1", ctx));
            assert_approx_eq(actual.bbox.y1, exp.bbox.y1, &format!("{} BBox y1", ctx));
            assert_approx_eq(actual.bbox.x2, exp.bbox.x2, &format!("{} BBox x2", ctx));
            assert_approx_eq(actual.bbox.y2, exp.bbox.y2, &format!("{} BBox y2", ctx));

            // Compare Landmarks
            match (&actual.landmarks, &exp.landmarks) {
                (Some(a_lms), Some(e_lms)) => {
                    assert_eq!(a_lms.len(), e_lms.len(), "{} Landmark count mismatch", ctx);
                    for (j, (a_pt, e_pt)) in a_lms.iter().zip(e_lms.iter()).enumerate() {
                        assert_approx_eq(a_pt.0, e_pt.0, &format!("{} Lmk {} x", ctx, j));
                        assert_approx_eq(a_pt.1, e_pt.1, &format!("{} Lmk {} y", ctx, j));
                    }
                }
                (None, None) => {}
                _ => panic!(
